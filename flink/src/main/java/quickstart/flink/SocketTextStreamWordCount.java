 ...
 *//***
	 ...
	 * text*socket.To run the example make sure that the service providing the
	 * text data*is already up and running.**
	 * <p>
	 * *To start an example socket text stream on your local machine run netcat
	 * from*a command line:<code>nc-lk 9999</code>,where the parameter specifies
	 * the*port number.***
	 * <p>
 ...
 * 
 * // set up the execution environment final StreamExecutionEnvironment env =
 * StreamExecutionEnvironment.getExecutionEnvironment();
 * 
 * // get input data DataStream<String> text = env.socketTextStream(hostName,
 * port);
 * 
 * DataStream<Tuple2<String, Integer>> counts = // split up the lines in pairs
 * (2-tuples) containing: (word,1) text.flatMap(new LineSplitter()) // group by
 * the tuple field "0" and sum up tuple field // "1" .keyBy(0).sum(1);
 * 
 * counts.print();
 * 
 * // execute program env.execute("WordCount from SocketTextStream Example"); }
 * 
 * // // User Functions //
 * 
 *//**
	 ...
	 * 
	 * @Override public void flatMap(String value, Collector<Tuple2<String,
	 * Integer>> out) { // normalize and split the line String[] tokens =
	 * value.toLowerCase().split("\\W+");
	 * 
	 * // emit the pairs for (String token : tokens) { if (token.length() > 0) {
